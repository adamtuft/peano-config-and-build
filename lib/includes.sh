...
# This file was generated by the Peano configuration script.
# It contains the module load commands and configuration for building Peano.
# build dir: $4
# branch:    $5 ($6)
...
__peano_configure_build() {
    ...
    if [[ -n "$save" ]]; then
        ...
        __peano_format_config_file "$module_commands" "$otter_commands" "$configure_commands" $(realpath "$peanodir") $(git branch --show-current) $(git rev-parse HEAD) | tee $save_file
